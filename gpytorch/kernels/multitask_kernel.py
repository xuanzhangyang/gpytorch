...
class MultitaskKernel(Kernel):
    ...
    def __init__(self, data_covar_module, num_tasks, rank=1, batch_shape=torch.Size(), task_covar_prior=None):
        super().__init__()
        self.task_covar_module = IndexKernel(
            num_tasks=num_tasks, batch_shape=batch_shape, rank=rank, prior=task_covar_prior
        )
    ...
    def size(self, x1, x2):
        ...
        """
        non_batch_shape = torch.Size([self.num_tasks * x1.size(-2), self.num_tasks * x2.size(-2)])
        # TODO: Broadcasting across batches?
        return self.batch_shape + non_batch_shape
